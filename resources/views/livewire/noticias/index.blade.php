...
new #[Layout('layouts.app')] class extends Component {
    ...
    {
        return [
            'data' => Noticia::where('titulo', 'LIKE', '%' . $this->search . '%')
...
<div>
    ...
    @if ($data->isEmpty())
        <div class="text-center mt-8">
            <p class="mb-4 text-2xl">Aún no hay noticias</p>
            <x-primary-button href="{{ route('noticias.create') }}" wire:navigate>Nuevo</x-primary-button>
        </div>
    @else
        <x-primary-button href="{{ route('noticias.create') }}" wire:navigate>Nuevo</x-primary-button>
                ...
                {{-- <div x-data="{ dropdown: false }" x-on:click.away="dropdown = false" class="relative">
                    ...
                    <div x-show="dropdown"
                        ...
                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                            ...
                            <li>
                                <a x-on:click="dropdown = !dropdown" href="{{ route('noticias.export', 'excel') }}"
                                    class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">excel</a>
                            </li>
                            <li>
                                <a x-on:click="dropdown = !dropdown" href="{{ route('noticias.export', 'pdf') }}"
                                    class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">pdf</a>
                            </li>
                            <li>
                                <a x-on:click="dropdown = !dropdown" href="{{ route('noticias.export', 'csv') }}"
                    ...
                    @foreach ($data as $item)
                        <tr wire:key="{{ $item->id }}"
                            ...
                            <td class="px-6 py-4 flex gap-4">
                                ...
                                <a href="{{ route('noticias.edit', $item) }}" wire:navigate
    ...
    @if ($show)
        <x-modal-show title="Detalle del noticia">
